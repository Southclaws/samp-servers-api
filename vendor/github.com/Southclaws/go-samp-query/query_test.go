...
func TestGetServerLegacyInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{"invalid", args{"not a valid url", false}, "failed to resolve: address not a valid url: missing port in address"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				...
				assert.NotEmpty(t, server.Gamemode)
				assert.NotZero(t, server.MaxPlayers)
			}
